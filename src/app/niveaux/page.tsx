        ...
        <div className="container mx-auto px-4">
          <div className="grid gap-12">
            {levels.map((level, index) => {
              // Réduction de qualité uniquement pour les 3 images lourdes
              const isHeavyImage = [
                "/images/Odyssee creche.jpeg",
                "/images/Groupe scolaire Odyssee.jpeg",
                "/images/College Jules-verne.jpeg",
              ].includes(level.image);
              ...
              return (
                ...
                >
                  {/* Image (plus petite + responsive) */}
                  <div
                    className={`relative rounded-2xl overflow-hidden shadow-xl ${
                      index % 2 === 1 ? "lg:order-2" : ""
                    } 
                    aspect-[16/10] md:aspect-[4/3] lg:aspect-[16/9]`}
                  >
                    <Image
                      src={level.image}
                      alt={level.name}
                      ...
                      // Indique des largeurs plus petites pour réduire la taille générée/chargée
                      sizes="(max-width: 768px) 100vw, (max-width: 1024px) 70vw, 40vw"
                      // Réduit le poids des 3 images ciblées
                      quality={isHeavyImage ? 55 : 70}
                      // Améliore la perf sur la première carte
                      priority={index === 0}
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                    <div className="absolute bottom-3 left-3 md:bottom-4 md:left-4">
                      <span className="bg-white/90 text-gray-900 px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-semibold">
                        {level.ageRange}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className={`space-y-6 ${index % 2 === 1 ? "lg:order-1" : ""}`}>
                    <div>
                      <h2 className="font-oswald text-3xl md:text-4xl font-bold text-gray-900 mb-3 md:mb-4">
                        {level.name}
                      </h2>
                      <p className="text-gray-600 text-base md:text-lg font-work-sans leading-relaxed">
                        {level.description}
                      </p>
                    </div>
                    ...
                    {/* Fonctionnalités */}
                    <div>
                      <h3 className="font-oswald text-lg md:text-xl font-semibold text-gray-800 mb-3 flex items-center gap-2">
                    ...
                    <div className="pt-2 md:pt-4">
                      <Link
                        href="/contact"
                        className="inline-flex items-center bg-gradient-to-r from-[#19559D] to-[#3BA65E] text-white font-semibold px-6 md:px-8 py-3 md:py-4 rounded-xl hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1"
                      >
